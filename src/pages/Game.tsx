...
export function Game({ joinCode, playerId, onLeaveGame }: GameProps) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const response = await fetch(
          `${SERVER_URL}/make-server-6ff8009f/validate`,
          {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    if (canMessItUp && gameState.game && !gameState.game.current_round_winner_id) {
      ...
      const claimRound = async () => {
        try {
          await fetch(
            `${SERVER_URL}/make-server-6ff8009f/games/${gameState.game?.id}/claim-round`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
  ...
  const fetchGameState = useCallback(async () => {
    try {
      ...
      if (USE_MOCK_BACKEND) {
        ...
        // Try the real server
        try {
          const url = `${SERVER_URL}/make-server-6ff8009f/games/${joinCode}/state?playerId=${playerId}`;
  ...
  useEffect(() => {
    if (gameState.game?.status === 'finished' && !showFinishedModal && finalStats.length === 0) {
      const fetchFinalStats = async () => {
        try {
          const response = await fetch(
            `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/final-stats`,
            {
              headers: {
                'Authorization': `Bearer ${publicAnonKey}`,
              },
  ...
  const handleStartGame = async () => {
    ...
    try {
      if (USE_MOCK_BACKEND) {
        ...
        // Use real server
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/start`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
      ...
      if (locationType === 'rack') {
        ...
        try {
          if (USE_MOCK_BACKEND) {
            ...
            await fetch(
                `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/move`,
                {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
  ...
  const handleMessItUp = async () => {
    ...
    try {
      if (USE_MOCK_BACKEND) {
        ...
        const response = await fetch(
          `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/split`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const handleStuck = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/stuck`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleFinishGame = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/finish`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleSendMessage = async (content: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/server/make-server-6ff8009f/games/${gameState.game?.id}/chat`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
